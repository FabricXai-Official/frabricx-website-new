...
import { Send, X, Minimize2, Maximize2, BookOpen, Lightbulb, MessageSquare, FileText } from "lucide-react";
import { FabricXaiKnowledgeBase, defaultFabricXaiContent } from "@/lib/knowledge-base";
import { FabricXaiAIAgent, ChatMessage as AIChatMessage } from "@/lib/ai-agent";
...
const EnhancedLiveChat = ({ className }: { className?: string }) => {
  ...
  const [aiAgent, setAiAgent] = useState<FabricXaiAIAgent | null>(null);
  ...
  useEffect(() => {
    const initializeAI = async () => {
      try {
        ...
        const openaiApiKey = process.env.NEXT_PUBLIC_OPENAI_API_KEY || 'your-api-key-here';
        
        if (openaiApiKey === 'your-api-key-here') {
  ...
  const handleSendMessage = async (message: string) => {
    ...
    try {
      ...
      if (aiAgent && aiAgent.isReady()) {
        // Use AI agent for intelligent response
        const aiResponse = await aiAgent.generateResponse(message, conversationHistory);
        ...
        assistantResponse = {
          ...
          sentTime: "just now",
          sender: "assistant",
          type: aiResponse.sources.length > 0 ? "sources" : "text",
          sources: aiResponse.sources,
          confidence: aiResponse.confidence,
        };

        // Add AI response to conversation history
        const aiAssistantMessage: AIChatMessage = { role: 'assistant', content: aiResponse.answer };
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-[#a8b0b7]">
              {isAILoading ? "Loading AI..." : aiAgent?.isReady() ? "AI-powered RMG solutions" : "RMG solutions"}
            </p>
          </div>
        </div>
              ...
              <div className="bg-[#242a30] text-white rounded-2xl px-3 py-2.5 sm:px-4 sm:py-3">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm text-[#a8b0b7]">
                    {aiAgent?.isReady() ? "AI is thinking..." : "Assistant is typing..."}
                  </span>
                </div>
              </div>
            </div>
        ...
        <div className="p-3 sm:p-4 border-t border-[#34383b] bg-[#1a2025]">
          <div className="flex gap-2">
            <input
              ...
              placeholder={aiAgent?.isReady() ? "Ask me anything about fabricXai..." : "Type your message..."}
          ...
          <div className="mt-2 text-center">
            {aiAgent?.isReady() ? (
              ...
              <p className="text-[#a8b0b7] text-xs">
                ⚡ <strong>Loading AI:</strong> Initializing knowledge base...
              </p>
            ) : (
