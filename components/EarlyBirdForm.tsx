...
const EarlyBirdForm: React.FC<React.DetailedHTMLProps<React.HTMLAttributes<HTMLDivElement>, HTMLDivElement>> = ({ className }) => {
    ...
    async function onSubmit(payload: EarlyBirdSchemaType) {
        ...
        try {
            ...
            if (!response.ok) {
                throw new Error(data.error || 'Failed to submit early bird request');
            }
                ...
                {success && (
                    <div className="mb-6 p-4 bg-green-900/20 border border-green-500 rounded-lg">
                        <p className="text-green-400 text-sm">
                            Thank you! Your early bird request has been submitted successfully. We&apos;ll be in touch soon with exclusive access details.
                        </p>
                    </div>
                )}
                        ...
                        <div className="pt-4">
                            ...
                            >
                                <span className="flex items-center justify-center gap-2">
                                    <span>{loading ? 'Submitting...' : 'Be an Early Bird'}</span>
                                    {!loading && (
